...
{{- with .main }}
{{- $main_context := . }}
{{- $containerList := (dict "k10_service_pod" $pod | include "get.serviceContainersInPod" | splitList " ") }}
...
{{- if eq (include "check.cacertconfigmap" .) "true" }}
      - name: {{ .Values.cacertconfigmap.name }}
        configMap:
          name: {{ .Values.cacertconfigmap.name }}
{{- end }}
{{- $containersInThisPod := (dict "k10_service_pod" $pod | include "get.serviceContainersInPod" | splitList " ") }}
