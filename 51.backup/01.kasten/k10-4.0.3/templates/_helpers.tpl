...
- job_name: {{ .k10service }}
  ...
  static_configs:
    - targets:
      ...
      - {{ .k10service }}-admin.{{ .main.Release.Namespace }}.svc.cluster.local:{{ $admin_port }}
      {{- else if eq "aggregatedapis" .k10service }}
      - {{ .k10service }}-svc.{{ .main.Release.Namespace }}.svc.cluster.local:443
      {{- else }}
      - {{ .k10service }}-svc.{{ .main.Release.Namespace }}.svc.cluster.local:{{ .main.Values.service.externalPort }}
      {{- end }}
      labels:
